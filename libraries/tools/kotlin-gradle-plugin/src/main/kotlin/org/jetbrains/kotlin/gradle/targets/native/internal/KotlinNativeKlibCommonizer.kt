...
internal data class CommonizerTaskParams(
    ...
    @Input
    fun getTargetNames() = targetGroups.map { it.map { target -> target.name } }

    init {
        destinationDirs = targetGroups.map { targets ->
            if (targets.size == 1) {
...
internal const val COMMONIZER_TASK_NAME = "commonizerTask"
